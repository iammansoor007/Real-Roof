...
async function checkData() {
    ...
    if (content?.data?.services?.services) {
        content.data.services.services.forEach((s: any, i: number) => {
            console.log(`Service ${i} (${s.title}): status=${s.status}`);
        });
    } else {
        console.log("No services found in data.services.services");
    }
    process.exit(0);
}
